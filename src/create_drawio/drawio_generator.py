...
# Allowed edge types
EDGES = [
    'ERmandOne', 'ERmany', 'ERone', 'ERoneToMany',
...
class CreateDrawio:
    ...
    def import_file(self, path_file_name: str):
        ...
        self.tables, self.references, self.positions = self._parse_dsl_file(path_file_name)
    ...
    def _parse_table_line(self, line: str) -> Tuple[str, Tuple[Optional[int], Optional[int]]]:
    ...
    def _parse_reference_line(self, line, tables, references):
        ...
        if match:
            src_table, src_col, tgt_table, tgt_col, start_arrow, end_arrow = match.groups()
    ...
    def _create_erd_xml(self) -> ET.Element:
        ...
        for table_name, columns in self.tables.items():
            if table_name in self.positions:
                ...
                root, width = self._create_table_xml(root, table_name, columns, x_offset)
                x_offset += width + 10
        return root
    ...
    def _add_columns(self, root, table_id, columns, width, height):
        ...
        for idx, (col_name, key) in enumerate(columns, start=1):
            ...
            self._create_row(root, row_id, table_id, fill_color, key, width, height, y_offset)
    ...
    def _create_row(self, root, row_id, parent_id, fill_color, key, width, height, y_offset):
    ...
    def _create_icon_cell(self, root, icon_id, parent_id, key, height):
        ...
        ET.SubElement(root, 'mxCell', {
            ...
            'vertex': '1',
            'parent': parent_id
        }).append(ET.Element('mxGeometry', {'width': '30', 'height': str(height), 'as': 'geometry'}))

    def _create_column_cell(self, root, col_id, parent_id, col_name, key, width, height):
    ...
    def write_mxgraph(self, file_name: str = 'output.drawio') -> None:
        ...
        graph_model = ET.Element('mxGraphModel', {
            'dx': '3247', 'dy': '533', 'grid': '0', 'gridSize': '10', 'guides': '1',
        ...
        ET.ElementTree(graph_model).write(file_name, encoding='utf-8', xml_declaration=True)

    # ADD EDGE
    def _add_edge(self, root: ET.Element, edge_id: str, source_id: str, target_id: str,
                start_arrow: str, end_arrow: str) -> ET.Element:
    ...
    'startArrow': start_arrow,
        ...
        if start_arrow and start_arrow not in EDGES:
            raise ValueError(f"Invalid start_arrow '{start_arrow}'. Allowed: {EDGES} or blank.")
        ...
        geometry = ET.SubElement(edge, 'mxGeometry', {'width': '100', 'height': '100', 'relative': '1', 'as': 'geometry'})
        ET.SubElement(geometry, 'mxPoint', {'x': '310', 'y': '98', 'as': 'sourcePoint'})
        ET.SubElement(geometry, 'mxPoint', {'x': '420', 'y': '230', 'as': 'targetPoint'})

        return root

    def _create_row_id(self, tables: Dict[str, List[Tuple[str, str]]], table_name: str, column_name: str) -> str:
    ...
    def _create_edges(self, root: ET.Element) -> ET.Element:
        ...
        for table, refs in self.references.items():
            for ref in refs:
                ...
                target_id = self._create_row_id(self.tables, ref['table_reference'], ref['column_reference'])
                edge_id = self._create_id()
                root = self._add_edge(root, edge_id, source_id, target_id, ref['start_arrow'], ref['end_arrow'])
        return root

    # ADD TITLE
    def add_title(self, root: ET.Element, title: str):
        style_title = self._dict_to_style_string({
    'shape': 'text',            # technically you might want to explicitly include this
    'strokeColor': 'none',
    'fillColor': 'none',
    'html': '1',
    'fontSize': '63',
